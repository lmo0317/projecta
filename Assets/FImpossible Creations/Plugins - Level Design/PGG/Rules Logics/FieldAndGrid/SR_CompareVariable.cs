...
namespace FIMSpace.Generating.Rules.FieldAndGrid
{
    public class SR_CompareVariable : SpawnRuleBase, ISpawnProcedureType
    {
        ...
        public SpawnerVariableHelper CompareVariable = new SpawnerVariableHelper(FieldVariable.EVarType.Float);

        public override List<SpawnerVariableHelper> GetVariables() { return CompareVariable.GetListedVariable(); }
        public override void GUIRefreshVariables() { CompareVariable.requiredType = FieldVariable.EVarType.Float; }
        ...
        {
            base.NodeBody(so);
            if (CompareVariable != null) CompareVariable.requiredType = FieldVariable.EVarType.Float;
