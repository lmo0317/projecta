        ...
        public InstantiatedData InstantiateStep()
        {
            ...
            if (spawnQueue.Count > 0)
            {
                ...
                if (spawnQueue[0] != null)
                {
                    ...
                    for (int s = 0; s < c.GetJustCellSpawnCount(); s++)
                    {
                        SpawnData sp = c.GetSpawnsJustInsideCell()[s];

                        if (sp.Prefab != null && sp.DontSpawnMainPrefab == false)
                        {
                            var container = InstantiatedInfo.GetContainerOf(sp.OwnerMod, true);

                            if (sp.OwnerCell == null) { sp.OwnerCell = Grid.GetCell(sp.OwnerCellPos); }
                            instantiated = InstantiatedData.InstantiateSpawnData(sp, RuntimeFieldSetup, container.Transform, container.Transform.localToWorldMatrix);
