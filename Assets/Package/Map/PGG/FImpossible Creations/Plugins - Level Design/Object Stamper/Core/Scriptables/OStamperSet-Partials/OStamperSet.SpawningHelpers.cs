...
namespace FIMSpace.Generating
{
    public partial class OStamperSet
    {
        ...
        {
            Vector3 l;
            Vector3 boundsSize = Vector3.Scale(info.ScaleOffsetMul, info.SetReference.ReferenceBounds.size);

            //if ( info.SetReference.ObjectShape == EOSType.Object)
            //{
            if (info.PrefabReference.FlatBounds)
            {
                l = (RayCheckDirection.normalized * boundsSize.magnitude) * RayDistanceMul;
            }
            else
            {
        ...
        {
            Vector3 boundsScale = Vector3.Scale(info.PrefabReference.ReferenceBounds.extents, info.ScaleOffsetMul);
            Vector3 boundsOffset = Vector3.Scale(boundsScale, directionNormalized);
            ...
            RaycastHit[] boundsHit = Physics.BoxCastAll(origin, boundsScale, directionNormalized, boundsRotation, boxCastDist, RayCheckLayer, QueryTriggerInteraction.Ignore);
